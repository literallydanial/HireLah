...
if (!function_exists('getCompanyLogoHtml')) {
    function getCompanyLogoHtml($employer_name, $company_logo) {
        try {
            $logo_clean = !empty($company_logo) ? trim((string)$company_logo) : '';
            if ($logo_clean !== '') {
                $has_logo = false;
                if (filter_var($logo_clean, FILTER_VALIDATE_URL)) {
                    $has_logo = true;
                } elseif (@file_exists(__DIR__ . '/' . ltrim($logo_clean, '/\\'))) {
                    $has_logo = true;
                }

                if ($has_logo) {
                    return '<div class="live-job-logo-box" style="background:#FFFFFF; border:1px solid #E5E7EB; padding:2px;"><img src="' . htmlspecialchars($logo_clean) . '" alt="' . htmlspecialchars((string)($employer_name ?? '')) . '" style="width:100%; height:100%; object-fit:contain; border-radius:11px;"></div>';
                }
            }

            $name_clean = trim((string)($employer_name ?? 'Company'));
            if ($name_clean === '') {
                $name_clean = 'Company';
            }
            ...
            if (function_exists('mb_substr') && function_exists('mb_strtoupper')) {
                $initial = mb_strtoupper(mb_substr($name_clean, 0, 1, 'UTF-8'), 'UTF-8');
            } else {
                $initial = strtoupper(substr($name_clean, 0, 1));
            }
            ...
            return '<div class="live-job-logo-box" style="background:#2563EB; color:#FFFFFF; font-size:18px; font-weight:800;">C</div>';
        }
    }
}
        ...
        <div class="keria-bottom-container">
            ...
            <div class="keria-live-jobs-card">
                ...
                <div class="live-jobs-right">
                    ...
                    <?php if (!empty($live_jobs) && is_iterable($live_jobs)): ?>
                        <?php foreach ($live_jobs as $job): 
                            try {
                                $job_id = (int)($job['id'] ?? 0);
                                $emp_name = !empty($job['employer_name']) ? (string)$job['employer_name'] : 'Keria Employer';
                                $job_title = !empty($job['job_title']) ? (string)$job['job_title'] : 'Open Position';
                                $emp_type = !empty($job['employment_type']) ? (string)$job['employment_type'] : 'Full-time';
                                $work_mode = !empty($job['work_mode']) ? (string)$job['work_mode'] : 'On-site';
                                $location = !empty($job['location']) ? (string)$job['location'] : (!empty($job['department']) ? (string)$job['department'] : 'Kuala Lumpur');
                                $time_ago = formatTimeAgo($job['created_at'] ?? null);
                        ...
                        <a href="jobs.php?id=<?= $job_id ?>" class="live-job-item" data-type="<?= htmlspecialchars($type_slug) ?>" data-workplace="<?= htmlspecialchars($mode_slug) ?>">
                            <div class="live-job-item-left">
                                <?= getCompanyLogoHtml($emp_name, $company_logo) ?>
